...
namespace {
   ...
   struct FrSky_msgID {
       ...
       static const char GPSLAT   =       0x1; // deg to int32_t
       static const char GPSLON   =       0x2; // deg to int32_t
       static const char GPSALT   =       0x3; // cm to uint32_t
   ...
   inline int16_t esc_write_sp(char * buf, int16_t len, bool start_of_frame)
   {
      ...
      for( ; pos < len; ++pos){
          ...
          if ( (ch == FrSky_msgID::header_value) || (ch == FrSky_msgID::escape_value) ){
             char ar[]= {FrSky_msgID::escape_value, static_cast<char>(ch ^ 0x60)};
             frsky_serial_port::write(ar,2);
             count += 2;
          }else{
             frsky_serial_port::write(buf + pos,1);
             ++count;
          }
      }
      return count;
